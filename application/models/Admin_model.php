...
class Admin_model extends CI_Model {
    ...
    public function generate_qucode($id, $type)
    {
        $this->load->library('ciqrcode');
        $qr_image= 'qrcode_'.$id.''.rand().'.png';
        $params['data'] = base_url().'readonly/'.$type.'/view/'.md5($id);
        $params['level'] = 'H';
        $params['size'] = 8;
        ...
        if($this->ciqrcode->generate($params))
        {
           ...
           $this->admin_model->edit_option($data, $id, 'invoice');
        }
        return;
    }
    ...
    {
        $this->db->select('i.*, b.id as biz_id, b.footer_note_align, b.phone as business_phone, b.color, b.name as business_name, b.address as business_address, b.template_style, b.logo, b.biz_number, b.vat_code, b.enable_qrcode, t.currency_symbol, t.name as country');
    ...
    {
        $this->db->select('i.*, b.id as biz_id, b.footer_note_align, b.phone as business_phone, b.uid, b.color, b.name as business_name, b.address as business_address, b.logo, b.biz_number, b.vat_code,  t.currency_code, b.enable_qrcode, t.currency_symbol, t.name as country');
        $this->db->from('invoice i');
        $this->db->join('business b', 'b.uid = i.business_id', 'LEFT');
        $this->db->join('country t', 't.id = b.country', 'LEFT');
        $this->db->where('md5(i.id)', $id);
    ...
    public function cron_recurring_payments(){

        $invoices = $this->get_cron_recurring_payments();
    ...
    // recurring invoices
    function get_cron_recurring_payments(){
        $this->db->select();
        $this->db->from('invoice');
        $this->db->where('recurring', 1);
        $this->db->where('is_completed', 0);
    ...
    // recurring invoices
    function get_complete_recurring_payments(){
        $this->db->select();
        $this->db->from('invoice');
        $this->db->where('recurring', 1);
        $this->db->where('is_completed', 0);
    ...
    function add_recurring_payments ($table, $primary_field, $primary_val) 
    {
       ...
       $query = $this->db->get($table);
        ...
        $recurring_data = array(
            ...
            'number' => get_auto_invoice_number(1, 1),
            ...
            'recurring_end' => $invoice->recurring_end,
    ...
    {   
        ...
        $user = $this->admin_model->get_by_id($invoice->user_id, 'users');
        ...
        $this->email_model->send($data['email_to'], $data['subject'], $data['html_content'], $data['email_myself'], $user->email);
    ...
    function get_user_reports(){
        ...
        if (isset($_GET['vendor']) && $_GET['vendor'] != 0) {
            $this->db->where('i.customer', $_GET['vendor']);
        }
    ...
    function get_tax_reports(){
        $this->db->select('i.*, t.name as tax_name, t.rate, s.currency');
        $this->db->from('invoice_taxes i');
        $this->db->join('tax as t', 't.id = i.tax_id', 'LEFT');
        $this->db->join('invoice as n', 'n.id = i.invoice_id', 'LEFT');
        $this->db->join('customers as s', 's.id = n.customer', 'LEFT');
        $this->db->where('t.business_id', $this->business->uid);
